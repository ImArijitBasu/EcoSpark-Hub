...
export default function AdminIdeasPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold font-[family-name:var(--font-heading)] text-white mb-6">Manage Ideas</h1>

      <div className="flex flex-wrap gap-2 mb-6">
        {['UNDER_REVIEW', 'APPROVED', 'REJECTED', 'DRAFT', ''].map((s) => (
          <button key={s} onClick={() => { setFilter(s); setPage(1); }}
            className={`px-4 py-2 rounded-xl text-xs font-medium cursor-pointer transition-all ${filter === s ? 'gradient-primary text-white' : 'glass-light text-dark-400 hover:text-white'}`}>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 glass rounded-2xl"><p className="text-dark-400">No ideas found</p></div>
      ) : (
      ...
      {feedbackModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="glass rounded-2xl p-6 w-full max-w-md">
            <h3 className="text-lg font-semibold text-white mb-4">Reject Idea</h3>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => handleReject(feedbackModal)} className="bg-red-500 hover:bg-red-600 text-white font-semibold px-6 py-2.5 rounded-xl flex-1 cursor-pointer transition-colors">Reject</button>
            </div>
          </div>
        </div>
      )}
